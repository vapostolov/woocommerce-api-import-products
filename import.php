...
//	$categories_json = parse_json( CATEGORIES_FILE_TO_IMPORT );
    ...
    if ($totalPages > 1) {
        for ($i = 2; $i <= $totalPages; $i++) {
            $wc_categories = array_merge($wc_categories, $woocommerce->get('products/categories', array("per_page" => 100, "page" => $i,)));
        }
    }
...
function get_products_and_variations_from_json($json, $added_attributes, $categories) {
    ...
    foreach ($json as $key => $pre_product) :
        ...
        if ($pre_product['type'] == 'simple') :
            ...
            if (isset($categories[$pre_product['category']])) {
            ...
            if (isset($pre_product['categories'])) {
                foreach ($pre_product['categories'] as $category_id) {
...
function get_categories_from_json($json) {
    ...
    foreach ($json as $key => $pre_product) :
        ...
        if (isset($pre_product['categories'])) {
            foreach ($pre_product['categories'] as $category_id) {
                $product_categories[$category_id] = null;
            }
        }
    endforeach;
